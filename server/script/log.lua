...
local function find(f, l) -- find element v of l satisfying f(v)
...
local function dump(o, seen, layer)
    if not seen then
        seen = {}
    end
    ...
    if type(o) == 'table' then
        ...
        for k,v in pairs(o) do
            ...
            s = s .. rep("\t", layer) .. '['..k..'] = ' .. dump(v, seen, layer) .. ',\n'
        end
        seen = {}

        local thisLayer = layer
        layer = layer - 1
        return s .. rep("\t", thisLayer - 1) .. '} '
    else
        o = tostring(o)
        return o
    end
