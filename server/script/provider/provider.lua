...
proto.on('textDocument/hover', function (params)
    ...
    log.info(hover.label)
    log.info(hover.description)
...
--             log.info(file)
--             local results = core.completion(uri, offset - 1, triggerCharacter)
    ...
    local function convert(symbol, fileUri)
        ...
        if symbol.children then
            for _, child in ipairs(symbol.children) do
                convert(child, fileUri)
            end
        end
    end

    local originDocumentUri   = params.textDocument.uri
    local symbols = core(originDocumentUri)

    if not symbols then
        symbols = {}
    else
        for _, symbol in ipairs(symbols) do
            convert(symbol, originDocumentUri)
        end
    end

    -- for _, fileUri in ipairs(files.getAllUris()) do
    --     local uriSymbols = core(fileUri)
    --     if not uriSymbols then
    --         return nil
    --     end

    --     if fileUri == originDocumentUri then
    --         goto CONTINUE
    --     end

    --     for _, symbol in ipairs(uriSymbols) do
    --         if symbol["name"]:find("Service:") or symbol["name"]:find("Service.Client:") then
    --             symbol = convert(symbol, originDocumentUri)--fileUri)
...
proto.on('workspace/symbol', function (params)
    ...
    log.info("WORKSPACE SYMBOL PARAMS")
        ...
        for _, visible in ipairs(visibles) do
            ...
            if piece then
                for _, edit in ipairs(piece) do
                    edits[#edits+1] = {
                        newText = edit.newText, -- "ooga booga", --
                        range   = files.range(uri, edit.start, edit.finish)
                    }
                end
            end
