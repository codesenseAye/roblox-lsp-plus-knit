...
local function buildInsertKnitModule(ast, moduleName, definitionPos, declarationPos, quotes, offset)
    local definitionStart
    if definitionPos then
        local uri = guide.getUri(ast.ast)
        local text  = files.getText(uri)
        local lines = files.getLines(uri)
        local row = calcline.rowcol(text, definitionPos)
        definitionStart = lines[row + 1].start
    end

    local declarationStart
    if declarationPos then
        local uri = guide.getUri(ast.ast)
        local text  = files.getText(uri)
        local lines = files.getLines(uri)
        local row = calcline.rowcol(text, declarationPos)
        declarationStart = lines[row + 1].start
    end

    local moduleType = moduleName:find("Controller") and "Controller" or "Service"

    if definitionStart and declarationStart then
        -- both declaration and definition positions were determined
        return {
            {
                start   = definitionStart,
                finish  = definitionStart - 1,
                newText = "\t" .. moduleName .. " = Knit.Get" .. moduleType .. "(\"" .. moduleName .. "\")\n"
            },
            {
                start   = declarationStart,
                finish  = declarationStart - 1,
                newText = "local " .. moduleName .. "\n"
            }
        }
    else
        -- no place to put definition or to put declaration found
        local uri = guide.getUri(ast.ast)
        local text  = files.getText(uri)
        local lines = files.getLines(uri)
        local row = calcline.rowcol(text, offset)
        local start = lines[row - 1].start
        local currentLine = lines[row]
...
local function checkKnitModule(ast, word, offset, results)
    local locals = guide.getVisibleLocals(ast.ast, offset)
    local definitionPosition
    local quotes = '"'

    log.info("MODULE URIS")

    -- find the other "moduleName = Knit.Get(moduleName)" lines and put this new one after the lowest one
    guide.eachSourceType(ast.ast, 'call', function (source)
        if source.node.field and (source.node.field[1] == "GetService" or source.node.field[1] == "GetController") then
            ...
            definitionPosition = source.start
        end
    end)

    local knitModuleUris = {}

    for _, uri in ipairs(files.getAllUris()) do
        if uri:find("Controllers/%w*Controller") or uri:find("Services/%w*Service") then
            local moduleName = uri:match("%w+Service") or uri:match("%w+Controller")
            knitModuleUris[moduleName] = uri
        end
    end

    -- find where to put the "local moduleName;" at
    local declarationPosition

    guide.eachSourceType(ast.ast, 'local', function (source)
        log.info("SET LOCAL")

        if knitModuleUris[source[1]] then
            log.info("SOURCE MATCHES KNIT MODULE")

            local setRef

            if not source.ref then
                -- this happens when there is a declaration but no definition
                return
            end
            ...
            declarationPosition = source.start
        end
    end)

    for moduleName, moduleUri in pairs(knitModuleUris) do
        if not locals[moduleName]
            and matchKey(word, moduleName)
        then
            log.info("MATCH")
            log.info(moduleName)
            results[#results+1] = {
                ...
                detail = "Import Knit Module \"" .. moduleName .. "\"",
                labelDetails = {
                    description = "Import Knit Module \"" .. moduleName .. "\"",
                },
                commitCharacters = {'.', ':'},
                id = stack(function ()
                    await.delay()
                    return {
                        additionalTextEdits = buildInsertKnitModule(ast, moduleName, definitionPosition, declarationPosition, quotes, offset),
                    }
                end)
            }
        end
...
local function checkKnitMethods(start, ast)
    ...
    local dotClientInCall = false

    if srcTableName and srcTableName == "Client" then
        dotClientInCall = true
        srcTableName = lookBackward.findWord(srcFullText, backwardPos - 2)
    end

    if dotClientInCall then
        isServer = false
    end
    ...
    if srcTableName and (srcTableName:find("Controller") or srcTableName:find("Service")) then
        ...
        guide.eachSourceType(uriAst.ast, "setmethod", function(source)
            local textInRange = knitModuleText:sub(source.start, source.finish)
            local dot = textInRange:find("%.")

            local methodName = textInRange:sub(textInRange:find(":") + 1, -1)
            local isClientMethod = false

            if dot then
                local fieldName = textInRange:sub(dot + 1, textInRange:find(":") - 1)
                -- log.info(methodName .. " + " .. fieldName)
            ...
            if isServer and not isClientMethod or not isServer and isClientMethod then
                table.insert(methodSources, {source, methodName, isClientMethod})
            end
        end)
    end
...
local function checkFieldThen(name, src, word, start, offset, parent, oop, text, results, infer, ast)
    ...
    -- prevent error before the user is done typing
    ...
    if oop then
        -- already used the "BuildFunction" function to put it in the results so no need to do it again
        -- unless it wasnt found in the current file
        log.info("is oop so return")
        return
    end
...
local function checkFieldOfRefs(refs, ast, word, start, offset, parent, oop, results, locals, isGlobal)
    ...
    local text = files.getText(ast.uri):sub(offset)
    local foundKnitMethods = checkKnitMethods(start, ast)
    for name, src in util.sortPairs(fields) do
        log.info(name)
        if src then
            checkFieldThen(name, src, word, start, offset, parent, oop, text, results, nil, ast)
        end
    end

    --incase some of the methods were not discovered by the "requestRefs" methods and etc ref methods,
...
-- auto fills table variables but not functions or methods
...
-- auto fill variables but not functions or methods
...
local function checkFunctionArgByDocParam(ast, word, start, results)
    ...
    func = func or guide.eachSourceContain(ast.ast, start, function (source)
                ...
                if afterLocal then -- local variable 
                    ...
                    if not isName(ast, offset) then
                        ...
                        if not isAfterFunction(text, start) then
                            ...
                            checkKnitModule(ast, word, start, results)
...
-- this is where function names get auto completed for sure fr fr fr fr
